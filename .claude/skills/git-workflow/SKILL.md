...
2. **不提交配置文件**：排除 `.env*`、`__pycache__/`、`*.pyc` 等配置和临时文件
...
git add module_admin/controller/xxx/
git add module_admin/service/xxx/
git add utils/xxx.py
# 或者添加具体文件
...
| scope | 说明 | 对应模块 |
|-------|------|----------|
| system | 系统管理 | `module_admin/` |
| demo | 演示模块 | `module_admin/demo/` |
| generator | 代码生成 | `module_generator/` |
| config | 配置模块 | `config/` |
| utils | 工具类 | `utils/` |
| common | 通用模块 | `module_admin/common/` |
...
git add module_admin/controller/feedback/
git add module_admin/service/feedback/
...
7. **Python 项目规范**：遵守 PEP 8 代码风格
8. **虚拟环境不提交**：venv/、env/ 等目录不提交
