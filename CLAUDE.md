...
基于 LangGraph 的 ReAct 模式，实现 LLM 驱动的自主 SQL 生成：

```
START → think → [tools → reflect → think]* → final_answer → END
```
...
| 文件 | 说明 |
|------|------|
| [app/agent_state.py](app/agent_state.py) | `NL2SQLAgentState` TypedDict，定义 Agent 状态 |
| [app/agent_tools.py](app/agent_tools.py) | 8 个 LangChain Tool（analyze_question, retrieve_knowledge, etc.） |
| [app/agent_nodes.py](app/agent_nodes.py) | ReAct 节点函数（think, tools, reflect, final_answer） |
| [app/agent_graph.py](app/agent_graph.py) | LangGraph StateGraph 构建，条件边 |
| [app/agent_routes.py](app/agent_routes.py) | Agent API 路由 |

**工具列表：**

| 工具 | 功能 | 参数 |
|------|------|------|
| `analyze_question` | 分析问题，提取意图、关键词、指标、维度 | question, datasource_id |
| `retrieve_knowledge` | 从知识库检索业务术语 | keywords |
| `retrieve_metrics` | 检索指标定义 | metric_names |
| `retrieve_metadata` | 检索表/字段映射和 M-Schema | topics, datasource_id |
| `get_schema` | 获取数据库 Schema（M-Schema 格式） | table_names, datasource_id |
| `generate_sql` | 生成 SQL 查询语句 | question, selected_tables, schema_context, previous_error |
| `execute_sql` | 执行 SQL 并返回结果 | sql, datasource_id |
| `explain_results` | 用自然语言解释查询结果 | question, sql, result |

**执行流程：**

1. **Think（思考节点）**：
   - LLM 分析当前状态，决定下一步行动
   - 通过 `llm.bind_tools()` 获取工具调用
   - 注入 tool_history 避免重复调用
...
| 端点 | 方法 | 说明 |
|------|------|------|
| `/agent/query` | POST | Agent 同步查询 |
| `/agent/stream` | GET | Agent SSE 流式查询 |
| `/agent/graph` | GET | 获取 Agent 图结构 |
